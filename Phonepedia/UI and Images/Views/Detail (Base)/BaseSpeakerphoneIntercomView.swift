    ...
    var body: some View {
        ...
        if phone.isCordless {
            Section("Multi-Device Calls") {
                if phone.supportsPBXFeatures {
                    InfoText("The options in this section pertain to this phone and its cordless devices, not communication between other compatible phones on the same line.")
                }
                if (phone.landlineConnectionType == 2 || phone.landlineConnectionType == 3) && (!phone.hasBaseSpeakerphone || phone.isCordedCordless) {
                    Picker("Cordless Device Line Behavior", selection: $phone.voIPCordlessDeviceLineBehavior) {
                        Text("Shares Same VoIP Line(s)").tag(0)
                        Text("Gets Separate Line").tag(1)
                        Text("Shared or Separate Lines").tag(2)
                    }
                    InfoText("• Shares Same VoIP Line(s): Every cordless device (and the base of a corded/cordless phone) can make and receive calls on any line/extension, just like a regular cordless phone. Intercom/call transfer doesn't rely on the VoIP connection.\n• Gets Separate Line: Each cordless device is assigned its own SIP account, which can be a line or PBX extension. Intercom/call transfer depends on the PBX/VoIP service being used.\n• Shared or Separate Lines: You can choose for each cordless device (and the base on a corded/cordless phone) whether you want it to have its own SIP account or to share all line(s) on the phone.")
                }
                if phone.voIPCordlessDeviceLineBehavior != 1 || phone.landlineConnectionType < 2 || phone.landlineConnectionType > 3 {
                    ...
                    InfoText("Intercom and call transfer depend on the VoIP service/PBX the phone is being used on.")
                }
            }
        }
    }
