...
struct LandlineDetailView: View {
    ...
    var body: some View {
        ...
        if phone.landlineConnectionType < 2 {
            ...
            InfoText("Business-grade VoIP phones often have support for more than 4 lines. Lines can be registered as extensions on the same VoIP system, or as separate lines/extensions on different VoIP systems.\nOn Ethernet VoIP phones, the number of lines DOES NOT determine how many line jacks the phone has. A single Ethernet cable connects the phone to the network, and the phone's software determines how many lines it can support. Additional Ethernet jacks, if any, are used to connect additional devices to the same network as the phone.")
        }
