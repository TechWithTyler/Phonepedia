...
• Number of rings: How many times the phone rings before the answering system answers a call. Selecting "Toll Saver" means the system answers after a certain number of rings if there are new messages (often 2 rings), or a few more rings if there are no new messages (often 4 rings). This allows you to know if you have new messages based on how many rings you hear, so you can hang up before the system answers without being charged for the call (when calling from a phone service that charges for outgoing calls). Another use for this feature is to save time by using the number of rings to determine whether you have new messages. Depending on the phone, this may be called "Time Saver" or "Auto" instead of "Toll Saver".
