...
struct AboutPhoneGradesView: View {
        ...
        NavigationStack {
            ...
            List {
                ...
                DisclosureGroup {
                    ...
                    Text("Some hotels use residential phones in their rooms. You can use a residential phone on a hotel line and vice versa as long as their connection types are the same (e.g. an analog residential cordless phone on a hotel room's analog line). If a hotel room's phone is a residential phone, there's a risk of guests changing settings or messing up the phone. Conversely, hotel phones, if they're VoIP, may have the ability to lock out all means of admin configuration (phone menus and web UI), including the ability to factory reset the phone. Full lockout of admin configuration should be used with caution, as it can permanently render the phone unusable if it can't connect to the network/VoIP system it was configured for.")
                    Text("Some residential/small-business and large-business phones have a hotel mode, which allows these phones to work like hotel phones.")
                } label: {
                    Text("Hotel Phone")
                }
                DisclosureGroup {
                    ...
                    Text("Large-business phones and hotel phones are used with a PBX (Private Branch Exchange) phone system, which can be a physical device with line ports, a server with phones and ATAs connected to it via the same local network or via the internet, or a cloud-hosted system with phones and ATAs connected to it via the internet. Physical PBXs and server-based PBXs that the business/user can manage themselves are called on-premise PBXs. See \"\(pbxSectionName)\" in the \"\(connectionTypesDialogTitle)\" dialog on the \"\(mainLinePageName)\" page to learn more.")
                    Button("Switch To \"About Connection Types/Devices\"…", systemImage: "arrow.right") {
                        switchToAboutConnectionTypes()
                    }
    ...
    // MARK: - Switch to "About Connection Types/Devices"

    func switchToAboutConnectionTypes() {
        dismiss()
