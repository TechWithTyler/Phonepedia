...
struct PhonebookRingtoneInfoView: View {
    ...
    var body: some View {
        InfoText("When a ringtone is assigned to a phonebook entry or favorite entry, that ringtone will be used when the entry calls. When a ringtone is assigned to a phonebook group, that ringtone will be used when any entry in the group calls unless the entry has a ringtone assigned. On analog phones, as caller ID isn't received until after the first ring, the main ringtone will be played first. Some phones will suppress the first ring when at least one phonebook entry has a ringtone assigned.\nFor phones which support silent mode and silent mode bypass, entries or groups can be selected to break through silent mode so their calls can still ring.\nSome phones have a VIP group/the ability to mark entries as VIPs. VIP entries can be assigned ringtones and be set to break through silent mode.")
    }
