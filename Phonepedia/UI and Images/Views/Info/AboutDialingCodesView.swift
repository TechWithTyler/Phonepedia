    ...
    var body: some View {
        NavigationStack {
            ...
            List {
                ...
                DisclosureGroup {
                    Text("The trunk prefix code (e.g., 1 for the US) is the digit(s) you may be required to dial before the area code when calling a number in your country. For example, in the US, you may be required to dial (925) 555-9876 as 1 (925) 555-9876.")
                } label: {
                    Text("Trunk Prefix Code")
                }
                DisclosureGroup {
                    Text("If you're using a phone on a PBX (private branch exchange) system, you need to dial one or more digits (e.g., 9) to tell it that you want to make a normal outside call, rather than an internal call to another one of its extensions. This is called the PBX line access number. Depending on how quickly the outside line is activated upon entering the code, when storing the line access number + outside phone number as a phonebook/speed dial/one-touch dial entry, you may need to add one or more pauses between the code and outside number. Pauses tell the phone to wait for a few seconds before dialing the next digit. The length of a single pause digit varies by phone, so you may have to use different numbers of pauses on different phones.")
                    ...
                    Text("Some PBXs allow you to dial the emergency number (e.g., 911 in the US or 999/112 in the UK) directly, without first dialing the line access number. Upon dialing the emergency number, the PBX would activate the outside line and \"repeat\" the dialed number's DTMF tones or pulses to it, so you may hear the number dialed again after you dial it.")
                    Text("If you want to reach a specific PBX extension when calling in from an outside phone, call the main number of the outside line the PBX is connected to. You'll be connected to an automated system or a live operator to help direct your call to the desired extension. You can use a PBX's automated system as a call block pre-screening system, as robots won't be able to enter the desired extension.")
                } label: {
