        ...
        NavigationStack {
            ...
            List {
                ...
                DisclosureGroup {
                    ...
                    Text("The trunk prefix code (e.g., 1 for the US) is the digit(s) you may be required to dial before the area code when calling a number in your country outside your local area code. For example, in the US, you may be required to dial \(examplePhoneNumber) as 1 \(examplePhoneNumber).")
                } label: {
                ...
                DisclosureGroup {
                    ...
                    Text("In hotels, the extension number of a room is either the same as the room number, or one or more digits and the room number. For example, if you want to call room \(NameNumberExamples.exampleHotelRoomNumber), you would dial one or more digits followed by \(NameNumberExamples.exampleHotelRoomNumber), or you would simply dial \(NameNumberExamples.exampleHotelRoomNumber), depending on the hotel's PBX system. Instructions in the room or on the phone's faceplate tell guests how to make room-to-room calls and, depending on the PBX, that they have to dial 9 before an outside phone number to make an outside call.")
                    ...
                    Text("If you want to reach a specific PBX extension when calling in from an outside phone, call the main number of the outside line the PBX is connected to. Depending on the system's configuration, you'll be connected to an automated system or a live operator to help direct your call to the desired extension. You can use a PBX's automated system as a call block pre-screening system, as robots won't be able to enter the desired extension.")
                } label: {
